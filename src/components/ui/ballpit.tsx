...
class x {
  ...
  #f() {
    if (this.#a) clearTimeout(this.#a as number);
...
const b = new Map<HTMLElement, any>(),
  A = new r();
let R = false;
function S(e: any) {
  const t = {
...
// Touch controls support standard pointer events on modern platforms, but fallback touch handlers are included.
// To avoid violating passive event listener constraints in modern browsers, we call preventDefault carefully.
...
class Z extends d {
  ...
  setColors(e: any) {
    if (Array.isArray(e) && e.length > 1) {
      const t = (function (colors: any) {
        let list: any[], i: any[];
        function setColors(colors: any) {
          list = colors;
          i = [];
          list.forEach(col => {
            i.push(new l(col));
        ...
        return {
          ...
          getColorAt: function (ratio: number, out = new l()) {
            const scaled = Math.max(0, Math.min(1, ratio)) * (list.length - 1);
            const idx = Math.floor(scaled);
            const start = i[idx];
            if (idx >= list.length - 1) return start.clone();
